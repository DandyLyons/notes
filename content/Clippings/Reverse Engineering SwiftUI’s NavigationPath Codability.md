...
tags:
  - clippings
  - article
description: Learn how to use Swift 5.7’s new existential type super powers to encode and decode  type-erased values.
takeaways:
  - This shows how to apply Codable to a collection that holds Any. I wonder if this technique can be broadened to be applied to a `[any MyProtocol]`. 
Status: ✅ Read
url: https://www.pointfree.co/blog/posts/78-reverse-engineering-swiftui-s-navigationpath-codability
---
...
Join us for a deep dive into some of Swift’s hidden runtime functions and Swift 5.7’s new existential tools so that we can reverse engineer `NavigationPath`’s codability. A compilable [gist](https://gist.github.com/mbrandonw/ed5d14b86e263fa6df008329cba74142) is available for following along.
...
Just as there is an underscored Swift function for turning a type into a string, there is also [one](https://github.com/apple/swift/blob/c8f4b09809de1fab3301c0cfc483986aa6bdecfa/stdlib/public/core/Misc.swift#L118-L127) that goes in the reverse direction, but it is failable because the string may not represent a type known to Swift:
...
Although `element` is partially erased to just an `any Encodable`, we can still access its dynamic runtime type with [`type(of:)`](https://developer.apple.com/documentation/swift/type\(of:\)). What if we could “peek” into the dynamic type of `value` and turn it into a static type `A`:
...
The name is called [`_openExistential`](https://github.com/apple/swift/blob/7cd3541c62276f53926bc6d0c9b3970d73dc2ea2/stdlib/public/core/Builtin.swift#L1003-L1016) because its job is to take a peek inside a type that we know nothing about in order to figure out its actual static type. The term “existential” is borrowed from predicate logic in mathematics, and if you want to know more how the mathematics is connected to the types, checkout [this week’s](https://www.pointfree.co/episodes/ep196-async-composable-architecture-tasks) episode.
