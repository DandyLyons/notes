---
docs: https://docs.godotengine.org/en/stable/tutorials/scripting/scene_unique_nodes.html
---
## Introduction[¶](https://docs.godotengine.org/en/stable/tutorials/scripting/scene_unique_nodes.html#introduction "Permalink to this headline")
...
## Creation and usage[¶](https://docs.godotengine.org/en/stable/tutorials/scripting/scene_unique_nodes.html#creation-and-usage "Permalink to this headline")

In the Scene tree dock, right-click on a node and select **Access as Unique Name** in the context menu.

![../../_images/unique_name.webp](https://docs.godotengine.org/en/stable/_images/unique_name.webp)

After selecting the option, the node will now have a percent symbol (**%**) next to its name in the scene tree:

![../../_images/percent.webp](https://docs.godotengine.org/en/stable/_images/percent.webp)
...
## Same-scene limitation[¶](https://docs.godotengine.org/en/stable/tutorials/scripting/scene_unique_nodes.html#same-scene-limitation "Permalink to this headline")
...
![../../_images/unique_name_scene_instance_example.webp](https://docs.godotengine.org/en/stable/_images/unique_name_scene_instance_example.webp)
...
## Alternatives[¶](https://docs.godotengine.org/en/stable/tutorials/scripting/scene_unique_nodes.html#alternatives "Permalink to this headline")
...
A [Group](https://docs.godotengine.org/en/stable/tutorials/scripting/groups.html#doc-groups) allows locating a node (or a group of many nodes) from any other node, no matter what scene the two nodes are located in.

A [Singleton (Autoload)](https://docs.godotengine.org/en/stable/tutorials/scripting/singletons_autoload.html#doc-singletons-autoload) is an always loaded node that can be accessed directly by any node regardless of the scene. These are useful when some data or functionality is shared globally.

[Node.find_child()](https://docs.godotengine.org/en/stable/classes/class_node.html#class-node-method-find-child) finds a node by name without knowing its full path. This seems similar to a scene unique node, but this method is able to find nodes in nested scenes, and doesn't require marking the node in the scene editor in any way. However, this method is slow. Scene unique nodes are cached by Godot and are fast to retrieve, but each time the method is called, `find_child()` needs to check every descendant (every child, grandchild, and so on).